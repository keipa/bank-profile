...
namespace BankProfiles.Web.Application.Features.Onboarding.Services;

public partial class BankOnboardingService : IBankOnboardingService
{
    ...
    private const string SqlCaseInsensitiveCollation = "SQL_Latin1_General_CP1_CI_AS";
    ...
    public async Task<BankOnboardingSubmissionResult> SubmitAsync(BankOnboardingSubmissionRequest request)
    {
        ...
        var duplicatePending = await context.BankOnboardingSubmissions
            .AnyAsync(s => s.Status == BankOnboardingStatuses.Pending
                ...
                && EF.Functions.Collate(s.ProposedBankName, SqlCaseInsensitiveCollation) == proposedBankName);
    ...
    public async Task<BankOnboardingApprovalResult> ApproveSubmissionAsync(BankOnboardingApprovalRequest request)
    {
        ...
        try
        {
            ...
            var bankCreatedInThisRequest = false;
            if (bank == null)
            {
                bank = new Bank
                {
            ...
            var existingCriteriaIds = bankCreatedInThisRequest
                ? new HashSet<int>()
                : await context.BankRatings
