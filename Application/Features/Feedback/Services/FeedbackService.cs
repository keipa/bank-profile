...
namespace BankProfiles.Web.Application.Features.Feedback.Services;
   ...
   : IFeedbackService
{
    ...
    public async Task<bool> SubmitFeedbackAsync(MetricFeedback feedback)
    {
        try
        {
            ...
            {
                logger.LogWarning("Invalid feedback submission from IP: {IP}", feedback.SubmitterIP);
                return false;
            }
            ...
            if (!string.IsNullOrEmpty(feedback.SubmitterIP))
            {
                ...
                {
                    logger.LogWarning("Rate limit exceeded for IP: {IP}", feedback.SubmitterIP);
                    return false;
                }
            }
            ...
            {
                logger.LogWarning("Feedback submission contains invalid bank code: {BankCode}", feedback.BankCode);
                return false;
            }
        ...
        {
            logger.LogError(ex, "Error submitting feedback from IP: {IP}", feedback.SubmitterIP);
            return false;
        }
    }
    ...
    public async Task<FeedbackModerationResult> ApproveFeedbackAsync(int feedbackId, string reviewedBy, string? reviewNotes)
    {
        ...
        try
        {
            ...
            if (!await eventStoreService.HasEventsAsync(bankCode))
            {
                var migrationResult = await eventMigrationService.MigrateSingleBankAsync(bankCode);
            ...
            var bankProfile = await bankDataService.GetBankByCodeAsync(bankCode);
            if (bankProfile == null)
            {
                return new FeedbackModerationResult
                {
        ...
        {
            logger.LogWarning(ex, "Feedback {FeedbackId} was already reviewed by another moderator.", feedbackId);
            return new FeedbackModerationResult
            {
                Success = false,
                Error = FeedbackModerationError.AlreadyReviewed,
        ...
        {
            logger.LogError(ex, "Database error while approving feedback {FeedbackId}.", feedbackId);
            return new FeedbackModerationResult
            {
                Success = false,
    ...
    public async Task<FeedbackModerationResult> RejectFeedbackAsync(int feedbackId, string reviewedBy, string? reviewNotes)
    {
        ...
        try
        {
            ...
            logger.LogInformation("Feedback {FeedbackId} rejected by {Reviewer}.", feedback.FeedbackId, reviewer);
        ...
        {
            logger.LogError(ex, "Database error while rejecting feedback {FeedbackId}.", feedbackId);
            return new FeedbackModerationResult
            {
                Success = false,
                Error = FeedbackModerationError.OperationFailed,
    ...
    public async Task<bool> CheckRateLimitAsync(string ipAddress)
    {
        ...
        {
            logger.LogError(ex, "Error checking rate limit for IP: {IP}", ipAddress);
            return false;
        }
    }
    ...
    public async Task<int> GetRemainingSubmissionsAsync(string ipAddress)
    {
        ...
        {
            logger.LogError(ex, "Error getting remaining submissions for IP: {IP}", ipAddress);
            return 0;
        }
    }
    ...
    public async Task CleanupOldSubmissionsAsync()
    {
        try
        {
            ...
            if (oldSubmissions.Count > 0)
            {
                ...
                logger.LogInformation("Cleaned up {Count} old feedback submissions", oldSubmissions.Count);
            }
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error cleaning up old submissions");
        }
    }
    ...
    private static string SanitizeHtml(string input)
    {
        ...
        var sanitizer = CreateInputSanitizer();
        var sanitized = sanitizer.Sanitize(input);
        return System.Net.WebUtility.HtmlDecode(sanitized);
    }

    private static HtmlSanitizer CreateInputSanitizer()
    {
    ...
    {
        cacheManager.Remove($"bank_{bankCode}");
        cacheManager.Remove(AllBanksCacheKey);
    }
}
