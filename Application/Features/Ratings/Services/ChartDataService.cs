...
namespace BankProfiles.Web.Application.Features.Ratings.Services;
...
public class ChartDataService : IChartDataService
{
    ...
    public async Task<List<ChartDataSet>> GetRatingHistoryByCriteriaAsync(string bankCode, int days = 30)
    {
        if (!ValidationHelper.IsValidBankCode(bankCode))
        {
            _logger.LogWarning("Invalid bank code attempted: {BankCode}", bankCode);
        ...
        try
        {
            ...
            var criteria = await context.RatingCriterias
                ...
                .Select(c => new { c.CriteriaId, c.Name })
                .ToListAsync();

            var historyData = await context.RatingHistories
                .AsNoTracking()
            ...
            var orderedDates = historyData
                .Select(h => h.Date)
                .Distinct()
                .OrderBy(d => d)
                .ToList();

            var labels = orderedDates
            ...
            foreach (var criterion in criteria)
            {
                var criteriaHistory = historyData
                    .Where(h => h.CriteriaId == criterion.CriteriaId)
                    .GroupBy(h => h.Date)
                    .ToDictionary(g => g.Key, g => (decimal?)g.Average(x => x.OverallRating));

                if (criteriaHistory.Count == 0)
                {
                    continue;
                }

                var color = GetColorForCriteria(criterion.CriteriaId);
                dataSets.Add(new ChartDataSet
                {
                    Label = criterion.Name,
                    Labels = labels,
                    Data = orderedDates
                        .Select(date => criteriaHistory.TryGetValue(date, out var rating) ? rating : null)
                        .ToList(),
                    BorderColor = color,
                    BackgroundColor = AddAlpha(color, 0.2)
                });
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error getting criteria rating history for bank {BankCode} and range {Days}",
                bankCode,
                days);
            return [];
        }
